...
export default function UserCardHeader({
  ...
  return (
    <div className="bg-gray-50 px-4 py-1.5 flex items-center gap-x-4 dark:bg-gray-700 dark:text-white">
        {showUser && <Image className="w-10 h-10 rounded-full" src={randomImage()} alt={username ? username : 'Random User'}/>}
        <div>
            <h3 className={`line-clamp-1 text-lg font-semibold tracking-tight ${headerFont.className}`} > {title} </h3>
